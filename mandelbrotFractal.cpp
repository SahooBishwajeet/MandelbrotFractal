...
// Mandelbrot Number is of the form "Z = Z**2 + c"
int isInMandelbrotSet(std::complex<double> c){
    ...
    for (int i = 0; i < 50; i++) {      // Change i < __ To Increase the Quality of render
        ...
        if(std::norm(z) > 16)           // 16 is completely arbitrary
            return i;                   // Returning 'i' will give us # of Iterations it took to go out of the bounds of Mandelbrot Set, 
                                        // which will decide the color of the pixel
    }
    return 0;
}
...
int main(int argc, char const *argv[])
{
    ...
    // Manderbrot Number is of the form "Z = Z**2 + c"
